...
function printResults(results) {
    ...
    results.forEach((result) => {
        ...
        if (result.unused.length > 0) {
            ...
            result.unused.forEach((u) => {
                console.log(
                    `  ${colors.red}Line ${u.line}:${colors.reset} ${colors.bold}${u.name}${colors.reset} ${colors.gray}from '${u.module}'${colors.reset}`
                );
            });
        }
    });
    ...
    if (totalUnused === 0) {
        ...
        console.log(
            `${colors.gray}Consider removing them to keep the codebase clean.${colors.reset}\n`
        );
    }
}
