...
async function loginHelper(
    ...
    try {
        ...
        // ═══════════════════════════════════════════════════════════════════════
        // DTSG AUTO-REFRESH SETUP
        // ═══════════════════════════════════════════════════════════════════════
        
        // Start automatic DTSG token refresh to prevent session expiration
        // This keeps the session alive for long-running bot instances
        const { startDtsgAutoRefresh, refreshDtsgToken, stopDtsgAutoRefresh } = utils;
        
        // Start auto-refresh (every 1 hour by default)
        const stopAutoRefresh = startDtsgAutoRefresh(ctx, defaultFuncs);
        
        // Expose refresh methods on API for manual control
        api.refreshDtsg = () => refreshDtsgToken(ctx, defaultFuncs);
        api.stopDtsgAutoRefresh = () => {
            stopAutoRefresh();
            stopDtsgAutoRefresh(ctx);
        };
        api.startDtsgAutoRefresh = (intervalMs) => startDtsgAutoRefresh(ctx, defaultFuncs, intervalMs);

        utils.success("DTSG AUTO-REFRESH ENABLED → Session will stay alive");
