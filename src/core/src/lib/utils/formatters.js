...
function _formatAttachment(attachment1, attachment2) {
    // Debug: Log raw attachment to understand structure
    // console.log('[DEBUG] Raw attachment1:', JSON.stringify(attachment1, null, 2));

    // Get blob_attachment - handle nested mercury.blob_attachment structure
    const blob_attachment =
        attachment1.mercury?.blob_attachment ||
        attachment1.blob_attachment ||
        attachment1.sticker_attachment ||
        attachment1.mercury; // Fallback to mercury if no blob_attachment
    ...
    if (
        ...
        if (
            ...
            return {
                ...
                // Extract URLs from blob_attachment
                ...
                isUnrecognized: false, // We successfully parsed it!
            };
        }

        // Try to detect if this is actually a photo based on other indicators
        ...
        // For unrecognized attachments, store ALL available fields that might contain photo info
        return {
            type: "share",
            ID: attachment1.id,
            ...
            source: null,
            isUnrecognized: true,
            // Store all potentially useful raw data
    ...
    switch (type) {
        ...
        default:
            // Return a generic attachment instead of throwing error
    ...
    return {
        ...
        attachments: (m.delta.attachments || []).map((v) => {
            try {
                ...
                // Error logged silently - attachment formatting failures are non-critical
                // Return a generic attachment object on error
                return {
                    type: "unknown",
                    ID: v?.id || v?.fbid || null,
