...
module.exports = {
    ...
    async execute({ api, event, config, logger }) {
        ...
        if (event.logMessageType) {
            logger.debug(
                "AntiLeave",
                `Received event type: ${event.type}, logMessageType: ${event.logMessageType}`
            );
        }
        ...
        if (wasKicked && config.isAdmin(authorID)) {
            logger.debug(
                "AntiLeave",
                `User ${leftParticipantFbId} was kicked by admin ${authorID}, not re-adding`
            );
            return;
        }
        ...
        try {
            ...
            logger.info(
                "AntiLeave",
                `Re-added ${userName} (${leftParticipantFbId}) to thread ${threadID}`
            );
        } catch (error) {
            // Common errors: user blocked bot, privacy settings, etc.
            logger.debug(
                "AntiLeave",
                `Failed to re-add user ${leftParticipantFbId}: ${error.message}`
            );
