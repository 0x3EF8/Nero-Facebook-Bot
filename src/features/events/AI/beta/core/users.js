    ...
    try {
        if (typeof api.getUserInfo === "function") {
            ...
            if (!info) {
                ...
                name =
                    entry && (entry.name || entry.fullName || entry.username || entry.displayName);
            } else if (typeof info === "object") {
                if (info[senderID]) {
                    const entry = info[senderID];
                    name =
                        entry &&
                        (entry.name || entry.fullName || entry.displayName || entry.username);
                } else {
...
async function getAllGroupMembers(api, threadID) {
    ...
    try {
        if (typeof api.getThreadInfo === "function") {
            ...
            if (
                threadInfo &&
                typeof threadInfo.userInfo === "object" &&
