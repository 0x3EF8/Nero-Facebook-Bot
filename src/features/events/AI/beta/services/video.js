...
Return JSON: {"optimizedQuery": "...", "contentType": "music video/tutorial/entertainment/etc"}`;
        ...
        if (fileSizeMB > 50) {
            ...
            api.setMessageReaction("❌", messageID, () => {}, true);
            return api.sendMessage(
                `⚠️ Video file too large (${fileSizeMB.toFixed(2)} MB). Max: 50 MB`,
                threadID,
                messageID
            );
        }
        ...
        console.log(
            chalk.green(
                ` ├─✓ Video sent: ${videoTitle.substring(0, 40)}... (${fileSizeMB.toFixed(2)} MB)`
            )
        );
        api.setMessageReaction("✅", messageID, () => {}, true);
        ...
        return api.sendMessage(
            "❌ Error downloading video. Try a different search query.",
            threadID,
            messageID
        );
    }
