    ...
    async execute({ api, event, args }) {
        ...
        if (!args.length) {
            ...
            return api.sendMessage(
                ...
                "Usage:\n" +
                `• ${actualPrefix}${commandName} <text> [-c <color>] - Generate styled QR with optional color\n` +
                `• ${actualPrefix}${commandName} <list> - Bulk generate (numbered list or newlines)\n\n` +
                "Options:\n" +
                "• -c, -color : Hex code (e.g. #FF0000) or name (e.g. orange)\n\n" +
                "Examples:\n" +
                `• ${actualPrefix}${commandName} https://google.com -c red\n` +
                `• ${actualPrefix}${commandName} -c orange\n` +
        ...
        if (!rawData) {
             return api.sendMessage("⚠️ Please provide text or data for the QR code.", threadID, messageID);
        }

        // Split by newline
        // Remove numbering (1., 2., 1), etc.) and trim
        const items = rawData
            .split(/\r?\n/)
        ...
        try {
            ...
            let messageBody = items.length > 1 
                ? `📱 Generated ${items.length} QR Codes`
                : (isName(items[0]) ? `📱 QR Name Card Generated` : `📱 QR Code Generated\n\n📝 Data: ${items[0].length > 50 ? items[0].substring(0, 50) + "..." : items[0]}`);
