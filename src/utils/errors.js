...
class ApiError extends BotError {
    ...
    static fromFacebook(error) {
        return new ApiError(error.message || "Facebook API request failed", "Facebook", error);
    }
}
...
const wrapError = (error, defaultMessage = "An unexpected error occurred") => {
    ...
    return new BotError(error.message || defaultMessage, "INTERNAL_ERROR", 500, {
