...
class CommandHandler {
    ...
    async handle(api, event) {
        ...
        try {
            ...
            // Check if it's a session error (1357004)
            if (error.errorCode === 1357004 || error.needsRefresh || errorMsg.includes("1357004")) {
                logger.warn(
                    "CommandHandler",
                    `Session error in command (${command.config.name}): ${errorMsg}`
                );

                // Attempt session refresh if available
                if (api.refreshSession) {
                    logger.info("CommandHandler", "Attempting automatic session refresh...");
                    try {
                        const refreshResult = await api.refreshSession();
                        if (refreshResult.success) {
                            logger.success("CommandHandler", "Session refreshed. Please retry the command.");
                            api.sendMessage(
                                "⚠️ Session was refreshed. Please try the command again.",
                                threadId
                            );
                        } else {
                            api.sendMessage(
                                "❌ Session expired and refresh failed. The bot may need to be restarted.",
                                threadId
                            );
                        }
                    } catch (refreshErr) {
                        logger.error("CommandHandler", `Session refresh failed: ${refreshErr.message}`);
                    }
                } else {
                    api.sendMessage(
                        "❌ Session expired. Please wait for automatic refresh or restart the bot.",
                        threadId
                    );
                }
            } else {
                logger.error(
                    "CommandHandler",
                    `Command error (${command.config.name}): ${errorMsg}`
                );
