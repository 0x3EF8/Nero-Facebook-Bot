...
class StatsTracker {
    ...
    getTopUsers(limit = 10) {
        return Array.from(this.userActivity.entries())
            .sort((a, b) => (b[1].messages + b[1].commands) - (a[1].messages + a[1].commands))
            .slice(0, limit)
            .map(([userId, data]) => ({
                userId,
                messages: data.messages,
                commands: data.commands,
                total: data.messages + data.commands,
            }));
    }
     ...
     * @returns {Object} Simple stats
     */
    getSimpleStats() {
        return {
            messages: this.stats.messages.total,
            commands: this.stats.commands.total,
            uptime: this.getUptime(),
            uptimeFormatted: this.getUptimeFormatted(),
        };
    }
    ...
    reset() {
        this.startTime = new Date();
        this.stats = {
            messages: { total: 0, text: 0, attachments: 0, reactions: 0 },
            commands: { total: 0, successful: 0, failed: 0, blocked: 0 },
            events: { total: 0, triggered: 0, failed: 0 },
        };
