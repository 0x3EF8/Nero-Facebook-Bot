        ...
        try {
            ...
            protocol.get(url, {
                ...
                if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
                    return downloadStream(response.headers.location, filename, type).then(resolve).catch(reject);
                }
                ...
                const finalFilename = type === 'photo' || type === 'animated_image' 
                    ? `photo_${Date.now()}.${ext}`
    ...
    try {
            ...
            if (!url) {
                ...
                if (photoId) {
                    try {
                        ...
                        if (resolvedUrl) {
                            const stream = await downloadStream(resolvedUrl, att.filename || "photo.jpg", "photo");
                            streams.push(stream);
                        }
                    } catch (e) {
        ...
        try {
            await api.sendMessage({
                body: msg,
                attachment: streams.length > 0 ? (streams.length === 1 ? streams[0] : streams) : undefined
            }, threadID);
        } catch {
