 ...
 * - Smart Reactions: Loading (⏳) and completion (✅/❌) indicators
...
async function fetchLyrics(songTitle, artist) {
  try {
    ...
    lyrics = lyrics.trim();
    lyrics = lyrics.replace(/^\d+\s+Contributors?.*?Lyrics/i, '');
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        line.match(/^\[(?:Verse|Chorus|Intro|Bridge|Pre-Chorus|Outro|Refrain)/i) ||
        (line.length > 0 && !line.includes('Read More') && !line.includes('Lyrics') && i < 10)
      ) {
        startIndex = i;
        break;
      }
...
async function downloadMusic(api, threadID, messageID, query, _model, wantsLyrics = false) {
  ...
  try {
    ...
    if (allVideos.length === 0) {
      ...
      let notFoundText = "❌ No music found for your search query.";
      try {
        ...
        console.warn(chalk.yellow("⚠ Failed to generate not found message"));
      }
    ...
    console.log(chalk.magenta(` ├─🧠 AI scoring ${allVideos.length} results for quality and relevance..."`));
...
Return ONLY valid JSON array, no markdown.`;
      ...
      if (scoringText) {
        ...
        console.log(chalk.green(` ├─✓ AI ranked results: #1 scored ${scores[0].score}% - ${scores[0].reason}`));
      }
    } catch (err) {
      console.warn(chalk.yellow("⚠ AI scoring failed, using default order:"), err.message);
    }
    ...
    for (const video of allVideos) {
      try {
        ...
        if (info.basic_info.duration <= 600 && info.basic_info.duration > 30) {
          ...
          console.log(chalk.green(` ├─✓ Found suitable music: ${videoTitle} (${Math.floor(info.basic_info.duration / 60)}:${String(Math.floor(info.basic_info.duration % 60)).padStart(2, '0')})`));
          break;
        }
      } catch {
        continue;
    ...
    if (!foundWithinLimit) {
      console.log(chalk.yellow(` ├─⚠ No suitable music in first results, generating AI alternatives...`));
...
Return ONLY valid JSON array, no markdown.`;
        ...
        if (altText) {
          ...
          console.log(chalk.cyan(` ├─🧠 AI generated ${aiAlternatives.length} smart alternatives`));
        }
      } catch {
        console.warn(chalk.yellow("⚠ AI alternative generation failed, using defaults"));
      }
        ...
        try {
          ...
          for (const video of altVideos) {
            ...
            if (info.basic_info.duration <= 600 && info.basic_info.duration > 30) {
              ...
              duration = selectedVideo.duration?.text || "N/A";
              foundWithinLimit = true;
              console.log(chalk.green(` ├─✓ Found alternative: ${videoTitle} (${Math.floor(info.basic_info.duration / 60)}:${String(Math.floor(info.basic_info.duration % 60)).padStart(2, '0')})`));
              break;
            }
          }
          ...
          console.warn(chalk.yellow(`⚠ Error in alternative search: ${err.message}`));
          continue;
        }
      }
    }
    ...
    if (!selectedVideo) {
      ...
      channelName = selectedVideo.author?.name || "Unknown";
      duration = selectedVideo.duration?.text || "N/A";
      console.log(chalk.yellow(` ├─⚠ Using first result despite duration`));
    }

    console.log(chalk.cyan(` ├─🎵 Found music: ${videoTitle} (${duration})`));
    ...
    if (info.basic_info.duration > 600) {
      console.log(chalk.yellow(` ├─⚠ Selected music is ${Math.floor(info.basic_info.duration / 60)} min, but will attempt download...`));
    }
    ...
    if (fileSizeMB > 50) {
      ...
      let tooLargeText = `⚠️ Audio file is too large (${fileSizeMB.toFixed(2)} MB)\n\n📏 Maximum file size: 50 MB`;
      try {
        ...
        console.warn(chalk.yellow("⚠ Failed to generate too large message"));
      }
...
Return ONLY valid JSON, no markdown.`;
        ...
        if (analysisText) {
          ...
          console.log(chalk.magenta(` ├─🧠 AI analyzed lyrics: Theme="${lyricsContext.theme}", Mood="${lyricsContext.mood}"`));
        }
      } catch {
        console.warn(chalk.yellow("⚠ AI lyrics analysis failed"));
      }
      
      // Generate enhanced AI lyrics intro with context
      let lyricsIntro = `📝 Lyrics\n\n🎵 ${videoTitle}\n👤 ${channelName}`;
      try {
        const contextInfo = lyricsContext 
          ? `This is a ${lyricsContext.mood} song about ${lyricsContext.theme}. ${lyricsContext.funFact}`
          : '';
        const lyricsPrompt = `Introducing lyrics for "${videoTitle}" by ${channelName}. ${contextInfo} Generate a very short intro (1 sentence) for the lyrics. Be casual and friendly. Use emojis.`;
        const lyricsResult = await model.generateContent(lyricsPrompt);
        lyricsIntro = lyricsResult?.response && typeof lyricsResult.response.text === "function"
          ? await lyricsResult.response.text()
          : lyricsIntro;
      } catch {
        console.warn(chalk.yellow("⚠ Failed to generate lyrics intro"));
      }
    ...
    // AI-POWERED MUSIC CAPTION with CONTEXT
    let musicCaption = `🎵 ${videoTitle}\nArtist: ${channelName}`;
    try {
      const contextInfo = lyricsContext 
        ? `This is a ${lyricsContext.mood} ${lyricsContext.theme} song. ${lyricsContext.funFact}`
        : '';
      const captionPrompt = `Sending music file "${videoTitle}" by ${channelName}. ${contextInfo} Generate a very short caption (1 sentence) for the music. Be casual and friendly. Use emojis.`;
      const captionResult = await model.generateContent(captionPrompt);
      musicCaption = captionResult?.response && typeof captionResult.response.text === "function"
        ? await captionResult.response.text()
        : musicCaption;
    } catch {
      console.warn(chalk.yellow("⚠ Failed to generate music caption"));
    }
    
    await api.sendMessage(
      {
        body: musicCaption.trim(),
    ...
    // AI-POWERED CONTEXTUAL MUSIC COMMENTARY
    try {
      const contextInfo = lyricsContext 
        ? `The song has a ${lyricsContext.mood} mood and is about ${lyricsContext.theme}. Fun fact: ${lyricsContext.funFact}`
        : '';
      const musicPrompt = `You just successfully sent a music file to the chat! The song is "${videoTitle}" by ${channelName}. ${contextInfo} Generate a short, fun message (1-2 sentences) about the song. Keep it casual and friendly. You can comment on the song, artist, mood, or just say enjoy!`;
      
      const musicResult = await model.generateContent(musicPrompt);
      const musicMessage = musicResult?.response && typeof musicResult.response.text === "function"
        ? await musicResult.response.text()
        : `🎵 Here's your music! Enjoy listening! 🎧`;
      
      await api.sendMessage(musicMessage.trim(), threadID, messageID);
    } catch {
      console.warn(chalk.yellow("⚠ Failed to generate music message"));
    }
    ...
    setTimeout(() => {
      try {
        ...
        console.error(chalk.yellow("⚠️ Error cleaning up audio file:"), _err.message);
      }
    }, 5000);
  } catch (error) {
    ...
    let errorMessage = "❌ Error downloading music. Try a different search query.";
...
Return ONLY valid JSON, no markdown.`;
      ...
      if (errorText) {
        ...
        console.log(chalk.magenta(` ├─🧠 AI diagnosed error: ${errorAnalysis.diagnosis}`));
        console.log(chalk.cyan(` ├─💡 AI suggests: ${errorAnalysis.suggestion}`));
        
        errorMessage = `❌ ${errorAnalysis.diagnosis}\n\n💡 ${errorAnalysis.suggestion}`;
        
        if (errorAnalysis.alternativeQuery && errorAnalysis.alternativeQuery !== query) {
          errorMessage += `\n\n🔄 Try: "${errorAnalysis.alternativeQuery}"`;
        }
      }
    } catch {
      console.warn(chalk.yellow("⚠ AI error diagnosis failed"));
    }
    
    if (error.message?.includes("413") || error.message?.includes("Payload Too Large")) {
      errorMessage = "❌ Audio file is too large for Messenger. Try searching for shorter songs.";
    } else if (error.message?.includes("403") || error.message?.includes("Forbidden")) {
      errorMessage = "❌ Audio download blocked by YouTube. Try a different song.";
    } else if (error.message?.includes("404") || error.message?.includes("Not Found")) {
      errorMessage = "❌ Music not found. The video may have been deleted.";
    }
    ...
    try {
      ...
      console.warn(chalk.yellow("⚠ Failed to generate error message"));
    }
  ...
  try {
    ...
    if (!geocodeResponse.data.results || geocodeResponse.data.results.length === 0) {
      ...
      return api.sendMessage(
        `❌ Location "${location}" not found. Please try a different location name.`,
        threadID,
        messageID
      );
    }
    ...
    const weatherDescription = weatherCodes[current.weathercode] || "🌡️ Unknown";
    ...
    for (let i = 0; i < 24; i += 3) {
      ...
      if (hourIndex < hourly.time.length) {
        ...
        forecast24h.push(`${time}: ${hourly.temperature_2m[hourIndex]}°C ${weatherCodes[hourly.weathercode[hourIndex]] || '🌡️'}`);
      }
    }
    ...
    for (let i = 0; i < Math.min(3, daily.time.length); i++) {
      ...
      forecast3day.push(`${date}: ${daily.temperature_2m_max[i]}°C/${daily.temperature_2m_min[i]}°C ${weatherCodes[daily.weathercode[i]] || '🌡️'}`);
    }
...
💨 WIND
  ...
  • Max Today: ${daily.windspeed_10m_max[0]} km/h
...
Data: Open-Meteo API • Time: Asia/Manila`;
...
async function downloadVideo(api, threadID, messageID, query, _model) {
  ...
  try {
    ...
    console.log(chalk.magenta(` ├─🧠 AI analyzing your video request..."`));
...
Return ONLY valid JSON, no markdown.`;
      ...
      if (analysisText) {
        ...
        if (searchStrategy.optimizedQuery && searchStrategy.confidence > 50) {
          ...
          console.log(chalk.green(` ├─✓ AI optimized query: "${optimizedQuery}" (confidence: ${searchStrategy.confidence}%)`));
        }
        
        if (searchStrategy.isAmbiguous && searchStrategy.suggestedAlternatives) {
          console.log(chalk.cyan(` ├─💡 AI suggests alternatives: ${searchStrategy.suggestedAlternatives.join(", ")}`));
        }
      }
    } catch (err) {
      console.warn(chalk.yellow("⚠ AI analysis failed, using original query:"), err.message);
    }
    ...
    if (allVideos.length > 0) {
      console.log(chalk.magenta(` ├─🧠 AI scoring ${allVideos.length} video results..."`));
...
Return ONLY valid JSON array, no markdown.`;
        ...
        if (scoringText) {
          ...
          console.log(chalk.green(` ├─✓ AI ranked videos: #1 scored ${scores[0].score}% - ${scores[0].reason}`));
        }
      } catch (err) {
        console.warn(chalk.yellow("⚠ AI video scoring failed, using default order:"), err.message);
      }
    }
    ...
    if (allVideos.length === 0) {
      ...
      let notFoundText = "❌ No videos found for your search query.";
      try {
    ...
    for (const video of allVideos) {
      try {
        ...
        if (info.basic_info.duration <= 600 && info.basic_info.duration > 30) {
          ...
          duration = selectedVideo.duration?.text || "N/A";
          foundWithinLimit = true;
          console.log(chalk.green(` ├─✓ Found suitable video: ${videoTitle} (${Math.floor(info.basic_info.duration / 60)}:${String(Math.floor(info.basic_info.duration % 60)).padStart(2, '0')})`));
          break;
        }
      } catch {
        continue;
    ...
    if (!foundWithinLimit) {
      console.log(chalk.yellow(` ├─⚠ No suitable video in first results, trying alternative searches...`));
    ...
    console.log(chalk.cyan(` ├─🎬 Found video: ${videoTitle} (${duration})`));
    api.setMessageReaction("⬇️", messageID, () => {}, true);
    ...
    if (info.basic_info.duration > 600) {
      console.log(chalk.yellow(` ├─⚠ Selected video is ${Math.floor(info.basic_info.duration / 60)} min, but will attempt download...`));
    }
    ...
    if (fileSizeMB > 100) {
      ...
      let tooLargeText = `⚠️ Video file is too large (${fileSizeMB.toFixed(2)} MB)\n\n📏 Maximum file size: 100 MB`;
      try {
        ...
        console.warn(chalk.yellow("⚠ Failed to generate video too large message"));
      }
    ...
    // Generate AI video caption
    let videoCaption = `🎬 ${videoTitle}\nChannel: ${channelName}`;
    try {
      const captionPrompt = `Sending video "${videoTitle}" from ${channelName}. Generate a very short caption (1 sentence) for the video. Be casual and friendly. Use emojis.`;
      const captionResult = await model.generateContent(captionPrompt);
      videoCaption = captionResult?.response && typeof captionResult.response.text === "function"
        ? await captionResult.response.text()
        : videoCaption;
    } catch {
      console.warn(chalk.yellow("⚠ Failed to generate video caption"));
    }

    await api.sendMessage(
      {
        body: videoCaption.trim(),
    ...
    // AI-POWERED VIDEO COMMENTARY with CONTEXT
    try {
      const contextInfo = searchStrategy?.contentType 
        ? `This is a ${searchStrategy.contentType} video.`
        : '';
      const videoPrompt = `You just successfully sent a video to the chat! The video is "${videoTitle}" from ${channelName}. ${contextInfo} Generate a short, fun message (1-2 sentences) about the video. Keep it casual and friendly. You can comment on the video, channel, content type, or just say enjoy watching!`;
      
      const videoResult = await model.generateContent(videoPrompt);
      const videoMessage = videoResult?.response && typeof videoResult.response.text === "function"
        ? await videoResult.response.text()
        : `🎬 Here's your video! Enjoy watching! 🍿`;
      
      await api.sendMessage(videoMessage.trim(), threadID, messageID);
    } catch {
      console.warn(chalk.yellow("⚠ Failed to generate video message"));
    }
    ...
    setTimeout(() => {
      try {
        ...
        console.error(chalk.yellow("⚠️ Error cleaning up video file:"), _err.message);
      }
    }, 5000);
  } catch (error) {
    ...
    // AI-POWERED ERROR DIAGNOSIS & RECOVERY
    let errorMessage = "❌ Error downloading video. Try a different search query.";
...
Return ONLY valid JSON, no markdown.`;
    ...
    if (error.message?.includes("413") || error.message?.includes("Payload Too Large")) {
      errorMessage = "❌ Video file is too large for Messenger. Try searching for shorter videos.";
    } else if (error.message?.includes("403") || error.message?.includes("Forbidden")) {
      errorMessage = "❌ Video download blocked by YouTube. Try a different video.";
    } else if (error.message?.includes("404") || error.message?.includes("Not Found")) {
...
module.exports.execute = async function({ api, event, config: _botConfig, logger: _logger }) {
    try {
      ...
      try {
        ...
        console.warn(chalk.yellow("⚠ Could not set loading reaction"));
      }

      // Validate API key (now from config.apiKeys.gemini)
      const apiKey = config.apiKeys?.gemini || config.geminiApiKey;
      if (!apiKey) {
        console.error(chalk.red("❌ Gemini API key not configured"));
        return;
      }
      ...
      console.log(chalk.cyan(` ├─👥 Group has ${allMembers.size} members`));
      ...
      if (text.toLowerCase().includes('change all') || text.toLowerCase().includes('everyone')) {
        console.log(chalk.magenta(` ├─📋 Members list for AI:\n${membersList}`));
      }
      ...
      if (hasImages) {
        ...
        if (imageParts.length > 0) {
          console.log(chalk.cyan(` ├─🖼️ Analyzing ${imageParts.length} image(s)`));
        }
      }
...
PAIRING TYPES:
1. "Pair me with someone" / "kinsa bagay sa akoa" / "who suits me" → Pair sender with random member
2. "Random pairs" / "pair everyone" / "pair random people" → Create random pairs from all members
...
3. Users ask for suggestions ("suggest music", "what should I listen to")
...
⚠️ CRITICAL: Always use REAL song titles with artist names, NOT generic descriptions!

Examples:
- User: "i'm so sad today" → MUSIC_SUGGESTION: Lewis Capaldi Someone You Loved | This beautiful ballad might help express what you're feeling 💔
- User: "need to study" → MUSIC_SUGGESTION: Lofi Girl study beats hip hop mix | Perfect focus music for your study session! 📚
- User: "i'm so happy!" → MUSIC_SUGGESTION: Pharrell Williams Happy | Let's match that energy! 🎉
- User: "beta suggest some music" → MUSIC_SUGGESTION: Dua Lipa Levitating | Here's a banger to brighten your day! ✨
- User: "can't sleep" → MUSIC_SUGGESTION: Marconi Union Weightless | This is scientifically proven to be relaxing 😴
- User: "feeling romantic" → MUSIC_SUGGESTION: Bruno Mars Just The Way You Are | Perfect love song vibes 💕
- User: "need workout music" → MUSIC_SUGGESTION: Eminem Lose Yourself | Let's get pumped! 💪
...
Do NOT add any other text. Just the command with the search query/location.`;
      ...
      const scheduleInstruction = `\n\n**🎓 INTELLIGENT ACADEMIC ASSISTANT - JAY'S SCHEDULE**:
...
- Day Type: ${isWeekend ? '🎉 WEEKEND (No Classes)' : isMWF ? '📚 MWF Schedule Day' : isTTH ? '📚 TTH Schedule Day' : '📚 School Day'}
- Current Minutes: ${currentTotalMinutes} (for time calculations)

📅 SCHEDULE DATABASE (SEPARATED BY PROGRAM):
...
- When user asks "IT schedule" or "IT sched" → ONLY show IT subjects, NEVER include CS!
- When user asks "CS schedule" or "CS sched" → ONLY show CS subjects, NEVER include IT!
- When user asks "schedule today" without specifying → Ask which program (IT or CS) OR show both clearly separated
- When user asks "next class" → Need to know if they're IT or CS student first, or ask!
...
5. PROACTIVE INTELLIGENCE:
   - If class is in < 30 min: "⚠️ Heads up! Class in X minutes!"
...
6. NATURAL RESPONSES - Don't just list data, be conversational and SMART:
   ❌ BAD: "10:30 AM - Theo 3b - Room 411" (too robotic)
   ❌ BAD: "Your next class is *Theo 3b*..." (no asterisks/markdown!)
   ✅ GOOD: "Your first class is Theo 3b (The Commandments) at 10:30 AM in Room 411. It's a theology class about moral principles - perfect time to reflect on ethics! ✨"
...
A: "📚 Here's your IT schedule for today (MWF):
→ 10:30 AM - Theo 3b (The Commandments) in Room 411
→ 11:30 AM - IT EL 1 (IT Elective 1) in Room 422
→ 4:00 PM - IT 312 (Analytics Modeling) in Room 409
That's 3 classes! Your first one starts at 10:30 AM ☕"

Q: "IT schedule today" (TTH day)
A: "📚 Here's your IT schedule for today (TTH):
→ 8:30 AM - IT 314 (Systems Admin) in ILLC
→ 2:00 PM - IT 311 (Analytic Tools) in ILLC  
→ 4:00 PM - IT 315 (Integrative Programming) in ILLC
→ 5:30 PM - IT EL 2 (IT Elective 2) in Room 407
→ 6:30 PM - IT 313 (Social Issues) in Room 409
Busy day! 5 IT classes, mostly in ILLC 🖥️"

Q: "CS schedule today" (MWF day)
A: "📚 Here's your CS schedule for today (MWF):
→ 10:30 AM - Theo 3b (The Commandments) in Room 411
→ 6:30 PM - CS 312 (Analytics Modeling) - TBA
→ 7:30 PM - CS 316 (Programming Languages) - TBA
3 classes! Long break between morning and evening 🌙"

Q: "next class?"
A: "Hey! 👋 Your next class is IT 312 (Analytics Modeling) at 4:00 PM in Room 409. That's in about 2 hours! It's a data science class where you'll work with predictive models and ML basics. Don't forget your laptop! 💻"

Q: "what's IT 314 about?"
A: "IT 314 (Systems Admin and Maintenance) is all about managing IT infrastructure! 🖥️ You'll learn Linux administration, server setup, network configuration, security hardening, and troubleshooting. It's hands-on work in the ILLC lab every Tuesday and Thursday at 8:30 AM. Super practical skills for any IT career!"

Q: "unsa akong IT sched karon?"
A: "Karon? Let me check... 😄 IT schedule nimo for today:
→ 8:30 AM - IT 314 sa ILLC
→ 2:00 PM - IT 311 sa ILLC
→ 4:00 PM - IT 315 sa ILLC
→ 5:30 PM - IT EL 2 sa Room 407
→ 6:30 PM - IT 313 sa Room 409
Lima ka IT classes! Busy day sa ILLC 🏃"

REMEMBER: You're not a schedule robot - you're an intelligent assistant who KNOWS the schedule AND understands academics. Provide context, tips, encouragement, and be helpful! 🎓`;
...
`;
      ...
      if (!responseText || !responseText.trim()) {
        console.warn(chalk.yellow("⚠ Empty AI response"));
        
        // Show error reaction
        try {
          api.setMessageReaction("❌", event.messageID, () => {}, true);
        } catch {
          console.warn(chalk.yellow("⚠ Could not set error reaction"));
        }
      ...
      if (musicSuggestionMatch) {
        ...
        // Generate AI "please wait" message
        let waitMessage = `⏳ Please wait, downloading "${searchQuery}" for you...`;
        try {
          const waitPrompt = `You're about to download the song "${searchQuery}" for the user. Generate a short, friendly "please wait" message (1 sentence) telling them you're downloading it. Be casual and use emojis. Examples: "Hold on, downloading it for you! 🎶", "Give me a sec, grabbing that track! ⏳🎵"`;
          const waitResult = await model.generateContent(waitPrompt);
          const waitText = waitResult?.response && typeof waitResult.response.text === "function"
            ? await waitResult.response.text()
            : null;
          if (waitText) {
            waitMessage = waitText.trim();
          }
        } catch {
          console.warn(chalk.yellow("⚠ Failed to generate wait message"));
        }
        
        // Send wait message
        await api.sendMessage(waitMessage, threadID, event.messageID);
        
        // Check if user wants lyrics (keywords: lyrics, lyric, letra, lirika)
        const wantsLyrics = /\b(lyrics?|letra|lirika)\b/i.test(text);
        
        // Then download the music
      ...
      if (musicMatch) {
        ...
        console.log(chalk.cyan(` ├─🎵 Music download request: "${searchQuery}"`));
      ...
      if (videoMatch) {
        ...
        console.log(chalk.cyan(` ├─🎬 Video download request: "${searchQuery}"`));
      ...
      if (weatherMatch) {
        ...
        console.log(chalk.cyan(` ├─🌤️ Weather request for: "${location}"`));
      ...
      if (datetimeMatch) {
        console.log(chalk.cyan(` ├─🕐 Date/Time request`));
        ...
        try {
          ...
          const dateTimeMessage = `🕐 Current Date & Time (Asia/Manila)

📅 ${fullDate}

🕐 12-Hour Format: ${time12h}
🕐 24-Hour Format: ${time24h}
...
📊 Additional Info:
  ...
  • Timezone: Asia/Manila (GMT+8)`;
          ...
          console.log(chalk.green(` ├─✓ Date/Time sent: ${fullDate} ${time12h}`));
          
          api.setMessageReaction("✅", event.messageID, () => {}, true);
        } catch (error) {
          console.error(chalk.red(` ├─✗ Date/Time error: ${error.message}`));
          api.setMessageReaction("❌", event.messageID, () => {}, true);
          await api.sendMessage("❌ Error getting date/time information.", threadID, event.messageID);
        }
      ...
      if (reply.includes('PAIR_ME')) {
        console.log(chalk.cyan(` ├─💑 Pairing sender with random member...`));
        
        const pairs = generateRandomPairs(allMembers, senderID, false);
        
        if (pairs.length === 0 || !pairs[0].person2) {
          await api.sendMessage(
            "❌ Not enough members in the group to create a pair!",
            threadID,
            event.messageID
          );
          api.setMessageReaction("❌", event.messageID, () => {}, true);
        ...
        try {
          ...
          const pairingMessage = pairingResult?.response && typeof pairingResult.response.text === "function"
            ...
            : `💑 ${pair.person1.name} ✨ ${pair.person2.name}\n\nLooks like you two are a match! 💕`;
          
          await api.sendMessage(pairingMessage.trim(), threadID, event.messageID);
          
          console.log(chalk.green(` ├─✓ Paired: ${pair.person1.name} with ${pair.person2.name}`));
        } catch {
          console.warn(chalk.yellow("⚠ Failed to generate pairing message"));
          await api.sendMessage(
            `💑 ${pair.person1.name} ✨ ${pair.person2.name}\n\nLooks like you two are a match! 💕`,
            threadID,
            event.messageID
      ...
      if (reply.includes('PAIR_RANDOM')) {
        console.log(chalk.cyan(` ├─💑 Creating random pairs for all members...`));
        
        if (allMembers.size < 2) {
          await api.sendMessage(
            "❌ Not enough members in the group to create pairs!",
            threadID,
            event.messageID
          );
          api.setMessageReaction("❌", event.messageID, () => {}, true);
        ...
        try {
          const pairsText = pairs.map((pair, index) => {
            if (pair.person2) {
              return `${index + 1}. ${pair.person1.name} 💕 ${pair.person2.name}`;
            } else {
              return `${index + 1}. ${pair.person1.name} ✨ (Solo)`;
            }
          }).join('\n');
          ...
          console.log(chalk.green(` ├─✓ Created ${pairs.length} random pairs`));
        } catch {
          console.warn(chalk.yellow("⚠ Failed to generate pairing message"));
          const pairsText = pairs.map((pair, index) => {
            if (pair.person2) {
      ...
      if (bulkMatch) {
        ...
        if (changes.length === 0) {
          ...
          console.log(chalk.yellow(` ├─💡 AI needs to use REAL user IDs from the members list, not array indexes`));
        ...
        try {
          ...
          console.warn(chalk.yellow("⚠ Failed to generate success message, using fallback"));
          await api.sendMessage(
        ...
        try {
          ...
          console.warn(chalk.yellow("⚠ Could not set success reaction"));
        }
      ...
      if (clearAllMatch) {
        ...
        for (const [userID, userName] of allMembers.entries()) {
          console.log(chalk.yellow(` ├─  Clearing nickname for ${userName} (${userID})`));
        ...
        try {
          ...
          console.warn(chalk.yellow("⚠ Failed to generate clear message, using fallback"));
          await api.sendMessage(
            `✨ Cleared ${successCount} nickname${successCount !== 1 ? 's' : ''}! Everyone's back to their default names. 🧹`,
            threadID,
            event.messageID
      ...
      if (clearMatch) {
        ...
        console.log(chalk.yellow(` ├─📝 Clear nickname request for: ${targetID}`));
        
        // Override with sender's ID if "my name" is mentioned
        const textLower = text.toLowerCase();
        if (textLower.includes('my name') || textLower.includes('my nickname')) {
          targetID = senderID;
          console.log(chalk.yellow(` ├─🔄 Override: Using sender's ID: ${targetID}`));
        }
        ...
        if (event.mentions && Object.keys(event.mentions).length > 0) {
          ...
          console.log(chalk.yellow(` ├─🔄 Override: Using mentioned user ID: ${targetID}`));
        }
        ...
        if (success) {
          console.log(chalk.green(` ├─✓ Nickname cleared successfully`));
          ...
          console.error(chalk.red(` ├─✗ Failed to clear nickname`));
      ...
      if (nicknameMatch) {
        ...
        console.log(chalk.yellow(` ├─📝 Original AI suggestion: targetID=${targetID}, nickname="${newNickname}"`));
        ...
        console.log(chalk.cyan(` ├─✏️ Final: Changing nickname for ${targetID} to "${newNickname}"`));
        console.log(chalk.cyan(` ├─📍 ThreadID: ${threadID}`));
        
        const success = await changeNickname(api, threadID, targetID, newNickname);
        
        if (success) {
          console.log(chalk.green(` ├─✓ Nickname change completed successfully`));
          ...
          console.error(chalk.red(` ├─✗ Nickname change failed`));
      ...
      try {
        ...
        console.log(chalk.cyan(` ├─🤖 Beta replied: "${reply.substring(0, 100)}..."`));
      } catch (sendErr) {
        console.error(chalk.red("❌ Failed to send message:"), sendErr.message);
        try {
          api.setMessageReaction("❌", event.messageID, () => {}, true);
        } catch {
          console.warn(chalk.yellow("⚠ Could not set error reaction"));
      ...
      console.error(chalk.red("❌ neroAI handler error:"), err.message || err);
