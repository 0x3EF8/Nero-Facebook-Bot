...
async function getWeather(api, threadID, messageID, location) {
  try {
    ...
    const searchMsg = await api.sendMessage(`🔍 Searching weather for: "${location}"...`, threadID);
    ...
    await api.editMessage(`✅ Found: ${placeName}\n\n⬇️ Getting weather data...`, searchMsg.messageID);
    ...
    for (let i = 0; i < 24; i += 3) {
      ...
      if (hourIndex < hourly.time.length) {
        ...
        forecast24h.push(`${time}: ${hourly.temperature_2m[hourIndex]}°C ${WEATHER_CODES[hourly.weathercode[hourIndex]] || "🌡️"}`);
      }
    }
    ...
    for (let i = 0; i < Math.min(3, daily.time.length); i++) {
      ...
      forecast3day.push(`${date}: ${daily.temperature_2m_max[i]}°C/${daily.temperature_2m_min[i]}°C ${WEATHER_CODES[daily.weathercode[i]] || "🌡️"}`);
    }
