...
function updateBotStats(data) {
  ...
  if (elements.botOnlineStatus && elements.botOnlineText) {
    if (bot && bot.online) {
      ...
      elements.botOnlineText.innerHTML = '<span style="color:#10b981;font-weight:600;">Online</span>';
    } else {
      elements.botOnlineStatus.className = 'cookie-health-status dead';
      elements.botOnlineText.innerHTML = '<span style="color:#ef4444;font-weight:600;">Offline</span>';
    }
  }

  // Messages stats
  if (messages) {
    if (elements.statMsgTotal) elements.statMsgTotal.textContent = messages.total?.toLocaleString() || '0';
    if (elements.statMsgText) elements.statMsgText.textContent = messages.text?.toLocaleString() || '0';
    if (elements.statMsgAttach) elements.statMsgAttach.textContent = messages.attachments?.toLocaleString() || '0';
    if (elements.statMsgReact) elements.statMsgReact.textContent = messages.reactions?.toLocaleString() || '0';
  }
  // Commands stats
  if (commands) {
    if (elements.statCmdTotal) elements.statCmdTotal.textContent = commands.total?.toLocaleString() || '0';
    if (elements.statCmdSuccess) elements.statCmdSuccess.textContent = commands.successful?.toLocaleString() || '0';
    if (elements.statCmdFailed) elements.statCmdFailed.textContent = commands.failed?.toLocaleString() || '0';
    if (elements.statCmdBlocked) elements.statCmdBlocked.textContent = commands.blocked?.toLocaleString() || '0';
  }

  // Activity stats
  if (activity) {
    if (elements.statActiveUsers) elements.statActiveUsers.textContent = activity.activeUsers?.toLocaleString() || '0';
    if (elements.statActiveThreads) elements.statActiveThreads.textContent = activity.activeThreads?.toLocaleString() || '0';
    if (elements.statTotalAccounts) elements.statTotalAccounts.textContent = activity.totalAccounts?.toLocaleString() || '0';
    // Top commands
    if (activity.topCommands && activity.topCommands.length > 0 && elements.topCommandsSection && elements.topCommandsList) {
      elements.topCommandsSection.style.display = 'block';
      elements.topCommandsList.innerHTML = activity.topCommands.map(cmd => `
  ...
  // Events stats
  if (events) {
    if (elements.statEventsTriggered) elements.statEventsTriggered.textContent = events.triggered?.toLocaleString() || '0';
  }

  // System stats
  if (system) {
    if (system.memory) {
      if (elements.statHeapUsed) elements.statHeapUsed.textContent = system.memory.heapUsed || '0 MB';
      if (elements.statHeapTotal) elements.statHeapTotal.textContent = system.memory.heapTotal || '0 MB';
      if (elements.statRss) elements.statRss.textContent = system.memory.rss || '0 MB';
    }
    if (elements.statNodeVersion) elements.statNodeVersion.textContent = system.nodeVersion || '-';
...
function _updateCookieHealth(validationData) {
  if (!validationData) {
    ...
    elements.cookieHealthStatus.innerHTML = '<div class="cookie-status-dot"></div><span>Unknown</span>';
  ...
  if (valid) {
    ...
    elements.cookieHealthStatus.innerHTML = '<div class="cookie-status-dot"></div><span>Healthy</span>';
    ...
    elements.cookieHealthStatus.innerHTML = '<div class="cookie-status-dot"></div><span>Dead/Invalid</span>';
    
    // Error message
    elements.validationMessage.className = 'validation-message error';
    elements.validationMessage.textContent = error || '✗ Cookies are expired or invalid. Please re-login to Facebook and extract new cookies.';
    elements.validationMessage.classList.remove('hidden');
...
async function extractCookies() {
  ...
  try {
    ...
    if (!tab) {
      ...
      elements.message.innerHTML = '<div class="message info"><span class="icon">🌐</span><span>Please open Facebook in the active tab to extract cookies.</span></div>';
      return;
    }
    ...
    elements.message.innerHTML = '<div class="message success"><span class="icon">✓</span><span>Cookies extracted successfully!</span></div>';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  elements.copyBtn.addEventListener('click', async () => {
    try {
      ...
      // Show success message
      elements.message.innerHTML = '<div class="message success"><span class="icon">✓</span><span>Copied to clipboard!</span></div>';
  ...
  function validateMainCookieInput() {
    ...
    if (!validation.valid) {
      inputEl.classList.add('error');
      const errors = validation.errors.slice(0, 3).map(e => `• ${e}`).join('<br>');
  ...
  elements.sendBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Cannot connect to server. Make sure Nero is running on ${NERO_CONFIG.serverUrl}`);
      }
      ...
      // Show success message
      elements.message.innerHTML = '<div class="message success"><span class="icon">✓</span><span>Cookies sent to Nero! Bot will restart automatically.</span></div>';
  ...
  function renderAccountsList(accounts) {
    elements.accountsList.innerHTML = accounts.map(account => {
      ...
      const profilePicUrl = account.profilePicUrl || `https://graph.facebook.com/${uid}/picture?width=100&height=100&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`;
      const _statusClass = account.status === 'online' ? 'status-online' : 'status-offline';
  ...
  async function performLogin() {
    ...
    if (isCustomMode) {
      ...
      try {
        ...
        loginIdentifier = validation.details.uid ? `UID: ${validation.details.uid}` : 'Custom Appstate';
      } catch (e) {
      ...
      try {
        ...
        const response = await fetch(`${NERO_CONFIG.serverUrl}/api/cookies/appstate?uid=${selectedAccountUid}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'x-api-key': NERO_CONFIG.apiKey
    ...
    try {
      ...
      if (successCount > 0) {
        showSyncNotification('success', `✓ Logged in with ${loginIdentifier}! Opening Facebook...`, 3000);
