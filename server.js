...
const { validateAppstate, formatAsAppstate } = require("./src/utils/cookieValidator");
 ...
 */
const getApiKey = (req) => {
    return req.headers["x-api-key"] || req.headers["authorization"]?.replace("Bearer ", "") || null;
};
...
const checkRateLimit = (req, res) => {
    ...
    if (!result.allowed) {
        const error = new RateLimitError(CONFIG.rateLimit.message, result.retryAfter, {
    ...
    handleCookies: async (req, res) => {
        try {
            ...
            if (saved) {
                ...
                logger.info(
                    "Server",
                    `New cookies received for UID: ${targetUid}. Restarting system...`
                );
                setTimeout(() => {
        ...
        if (uid) {
            ...
            if (!isValidUid(uid)) {
                const error = new ValidationError(
                    "Invalid UID format. Must be a 10-20 digit number.",
                    { uid: "invalid format" }
                );
...
const startServer = (silent = false) => {
    server.listen(CONFIG.port, CONFIG.host, () => {
        ...
        if (!silent && config.server?.logStartup !== false) {
            ...
            logger.info(
                "Server",
                `  Rate Limiting: ${CONFIG.rateLimit.enabled ? `${CONFIG.rateLimit.maxRequests} req/${CONFIG.rateLimit.windowMs / 1000}s` : "DISABLED"}`
            );
