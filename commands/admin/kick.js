...
module.exports.execute = async function({ api, event, args, logger }) {
    ...
    if (removeAllMembers) {
        try {
            ...
            const botID = api.getCurrentUserID();
            let allParticipantIDs = threadInfo.participantIDs.filter(id => 
                id.toString() !== botID.toString() && id.toString() !== senderID.toString()
            );
            
            // If excludeMembers is true, don't remove excluded users
            if (excludeMembers && excludedIDs.length > 0) {
                allParticipantIDs = allParticipantIDs.filter(id => !excludedIDs.map(e => e.toString()).includes(id.toString()));
            }
    ...
    if (removeAllMembers) {
        try {
            ...
            if (updatedThreadInfo && updatedThreadInfo.participantIDs) {
                ...
                const othersLeft = remainingMembers.filter(id => 
