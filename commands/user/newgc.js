 ...
 */
module.exports.execute = async function({ api, event, args, logger: _logger }) {
    ...
    try {
        ...
        let creatingMsg = `⏳ Creating group${groupName ? ` "${groupName}"` : ""}...\n` +
        ...
        if (result && result.success) {
            ...
            if (api.changeAdminStatus) {
                try {
                    ...
                    const adminResult = await api.changeAdminStatus(result.threadID, senderID, true);
            ...
            const welcomeMsg = `👋 Welcome to${groupName ? ` "${groupName}"` : " the group"}!\n\n` +
