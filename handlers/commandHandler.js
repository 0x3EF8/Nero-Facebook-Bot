...
class CommandHandler {
    ...
    async init() {
        ...
        logger.success("CommandHandler", `Loaded ${this.stats.loaded} commands from ${directories.length} categories`);
    ...
    async reloadCommand(commandName) {
        ...
        try {
            ...
            logger.error("CommandHandler", `Failed to reload command ${commandName}: ${error.message}`);
            return false;
        }
    }
    ...
    async handle(api, event) {
        ...
        if (!(config.commands.ignoreCooldownForAdmins && config.isAdmin(userId))) {
            ...
            if (remaining > 0) {
                api.sendMessage(`⏳ Please wait ${remaining} seconds before using this command again.`, threadId);
                return false;
            }
        }
        
        // Note: Typing indicator is handled by humanBehavior in the framework
        // when humanBehavior is enabled - no need for manual typing indicator here
        
        // Execute command
        try {
            // Log command execution with details
            const argsStr = args.length > 0 ? `args=[${args.join(", ")}]` : "args=[]";
            logger.info("CommandHandler", `Executing: ${command.config.name} │ user:${userId} │ thread:${threadId} │ ${argsStr}`);
            ...
            logger.error("CommandHandler", `Command error (${command.config.name}): ${error.message}`);
